    ...
    run_mongo_etl = EmrServerlessStartJobOperator(
        ...
        job_driver={
            "sparkSubmit": {
                ...
                # Crucial additions:
                # 1. --packages download the official Spark MongoDB connector
                # 2. Includes the 3 core PySpark libraries as dependencies (--py-files)
                "sparkSubmitParameters": f"--py-files s3://{S3_BUCKET}/scripts/schema_validator.py,s3://{S3_BUCKET}/scripts/data_quality.py,s3://{S3_BUCKET}/scripts/encryption_utils.py --packages org.mongodb.spark:mongo-spark-connector_2.12:10.3.0 --conf spark.executor.cores=4 --conf spark.executor.memory=14g --conf spark.driver.cores=4 --conf spark.driver.memory=14g --conf spark.dynamicAllocation.enabled=false --conf spark.sql.shuffle.partitions=8 --conf spark.sql.adaptive.enabled=true --conf spark.hadoop.fs.s3.enableServerSideEncryption=true --conf spark.hadoop.fs.s3.serverSideEncryption.kms.keyId={KMS_KEY_ARN}"
            }
        },
